...
import { List, InputGroup, PickerGroup, TextareaGroup, RadioGroup, DatePickerGroup, MixPickerGroup } from '@/components/LianForm';
import PageWrapper from '@/components/PageWrapper';
import StepsBar from '@/components/StepsBar';
import styles from '@/pages/perinatal/index.less';

@connect(({ perinatal, perinatalInfo }) => ({
  fieldLabels: perinatalInfo.fieldLabels,
  perinatalList: perinatal.dataSource
}))
class PerinatalInfo extends Component {
  static propTypes = {
    form: formShape,
  };
  state = {};
  ...
  submit = () => {
    this.props.form.validateFieldsAndScroll((error, value) => {
      const { fieldLabels } = this.props;
      console.log('error', error, value);
      if (error) {
        const firstKey = Object.keys(error)[0];
        const message = error[firstKey]['errors'][0]['message'];
  ...
  render () {
    ...
    return (
      <PageWrapper goBack>
        <WingBlank >
          <StepsBar dataSource={perinatalList} current={1} />
        </WingBlank>
...
export default createDOMForm()(PerinatalInfo);
